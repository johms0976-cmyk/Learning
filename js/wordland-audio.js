/* ============================================================
   ...
   before, during and after you finish recording.
     ...
     audio/wordland/story/map2-chapter1.mp3   ... and map 2 onward

   Map 1's chapter files keep their original names, so anything
   already recorded still plays.
...
const WLAudio = (function () {
  ...
  async function init() {
    ...
    const sentinels = [
      ...
      url('story', chapterName(first.map, first.no))
    ];
  ...
  return {
    init, rescan, unlock, stop, preloadNode, preloadMap, expected, report,
    mode: () => mode,
