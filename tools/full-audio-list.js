...
const out = { sounds:new Map(), letters:new Map(), words:new Map(),
              story:new Map(), phrases:new Map(), praise:new Map(),
              players:new Map(), names:new Map() };
const KINDS = ['sounds','letters','words','story','phrases','praise','players','names'];
...
/* Every rising line needs a finished twin. The rising one is only
   heard when the child's name is recorded; when it isn't, the game
   plays the phrases/ version so the sentence doesn't hang. */
PRAISE_LINES.forEach(l => add('phrases', slug(l), {
  say: l, from: 'praise, said on its own',
...
while ((m2 = zibRe.exec(zibBlock))) {
  ...
  if (isName) add('phrases', slug(text), {
    say: text, from: 'Zib · ' + group + ', said on its own',
    note: 'the finished version — heard when the name is not recorded'
  });
}
...
/* ── 8. THE NAME LIBRARY ─────────────────────────────────────
   audio/wordland/names.json, written by tools/names-list.js.
   Missing is fine — it just means no names have been chosen yet. */
const nameSlug = s => String(s == null ? '' : s)
  .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
  .toLowerCase().trim()
  .replace(/['’`]/g, '').replace(/[\s._]+/g, '-')
  .replace(/[^a-z0-9-]/g, '').replace(/-+/g, '-').replace(/^-|-$/g, '');

const namesJson = path.join(R, 'audio/wordland/names.json');
let NAMES = [];
if (fs.existsSync(namesJson)) {
  try { NAMES = JSON.parse(fs.readFileSync(namesJson, 'utf8')) } catch (e) {
    console.error('!! audio/wordland/names.json is not valid JSON');
  }
}
...
function haveIn(pack) {
  const set = new Set();
  const base = path.join(R, 'audio/wordland', pack ? 'voices/' + pack : '');
  KINDS.forEach(k => {
    const dir = path.join(base, k);
    if (fs.existsSync(dir)) fs.readdirSync(dir).filter(f => f.endsWith('.mp3'))
      .forEach(f => set.add(k + '/' + f.replace(/\.mp3$/, '')));
  });
  return set;
}
const have = haveIn('');

/* Every voice pack that has a folder, plus the four we expect. */
function packs() {
  const dir = path.join(R, 'audio/wordland/voices');
...
if (require.main === module) {
  ...
  const mine = pack ? haveIn(pack) : have;
  if (pack) console.log('pack: ' + pack + '\n');
  let total = 0, done = 0;
  for (const k of KINDS) {
    const names = [...out[k].keys()].sort((a,b)=>a.localeCompare(b,'en',{numeric:true}));
    const d = names.filter(n => mine.has(k + '/' + n)).length;
