...
function buildRounds(n){
  ...
  const rounds = normalisePlan(n).map(entry => {
    ...
    if(type==='beginSound'){
      ...
      const onFocus = n.vocab.filter(v => focus.some(L => hits(v, L)) && !used.pic.includes(v.w));
    ...
    if(type==='starts'){
      ...
      const onFocus = n.vocab.filter(v => focus.some(L=>hits(v,L)) && !used.pic.includes(v.w));
      const spare = focus.flatMap(L => picsWith(L, n, atEnd)).filter(v => !used.pic.includes(v.w));
      ...
      const wrong = shuffle(ALL_PICS.filter(v =>
        !(atEnd ? v.w.endsWith(L) : v.w.startsWith(L)) && v.w!==right.w)).slice(0,3);
...
let run = { node:1, rounds:[], i:0, mistakes:0, missThis:0, busy:false, startedAt:0,
            firstTry:0, planned:0, requeued:0, trouble:{} };
