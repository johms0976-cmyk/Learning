...
/* ── the picture of a word ───────────────────────────────────
   Never print v.e directly. A word with a drawing in PICTURE
   must show the drawing, and only wordland-pictures.js knows
   which words those are. picHTML() returns the markup; pkey()
   returns whatever identifies that picture, for the filters
   that stop two cards in a row looking the same.

   Both fall back to the raw emoji if wordland-pictures.js has
   not loaded, so a missing file degrades to the old behaviour
   rather than a blank screen. Named picHTML, not pic, because
   two builders below already use `pic` for a local.           */
const picHTML = (v, cls) => (typeof pictureHTML === 'function')
  ? pictureHTML(v, cls)
  : `<span class="pic-emoji${cls ? ' ' + cls : ''}">${(v && v.e) || ''}</span>`;
const pkey = v => (typeof picKey === 'function') ? picKey(v) : ((v && v.e) || '');
...
function buildRounds(n){
  ...
  const rounds = normalisePlan(n).map(entry => {
    ...
    if(type==='tapAll'){
      ...
      const taken = new Set(right.map(r => pkey(r)));
      const wrong = [];
      shuffle(picturable(ALL_PICS.filter(v =>
          !has(v,chosen.L) && !right.some(r=>r.w===v.w)), 6)).forEach(v => {
        if(wrong.length >= 3 || taken.has(pkey(v))) return;
        taken.add(pkey(v)); wrong.push(v);
      });
    ...
    if(type==='match'){
      ...
      const pairs = letters.map(L => {
        const av = picturable(ALL_PICS.filter(v => v.w[0]===L && !seenPic.has(pkey(v))), 1);
        const pic = rnd(av.length ? av : ALL_PICS.filter(v => v.w[0]===L));
        if(pic) seenPic.add(pkey(pic));
        return { l:L, pic };
      });
    ...
    if(type==='sentence'){
      const s = nextSent();
      return { type, words:s.s, pic:s.e, picW:s.s[s.s.length-1], tiles:shuffle(s.s) };
    }
    ...
    if(type==='pickWord'){
      ...
      return { type, words:s.s, at, answer, pic:s.e, picW:s.s[s.s.length-1], opts:shuffle([answer, wrong]) };
    }
    ...
    if(type==='readLine'){
      ...
      const wrong = shuffle(ALL_PICS.filter(v => pkey(v) !== pkey(right))).slice(0,2);
    ...
    if(type==='blendIt'){
      const w = nextSoundable();
      const units = splitGraphemes(w.w);
      const wrong = shuffle(ALL_PICS.filter(v => v.w !== w.w && v.e && pkey(v) !== pkey(w))).slice(0,2);
...
function interleaveReview(n, rounds, wrongLetters){
  ...
  if(src.words.length >= 3){
    ...
    const wrong = shuffle(src.words.filter(v => v.w !== w.w && pkey(v) !== pkey(w))).slice(0,2);
    if(wrong.length === 2)
...
function win(word){
  ...
  $('cheerE').classList.toggle('small', big);
  ...
  /* The cheer is Zib talking, so Zib is on screen saying it. He
     comes up for the real ones only — for the quick ticks he
     would be wallpaper inside a minute. */
  const heard = big ? WLAudio.praise(c[1], 0.85, 1.2) : null;
  if(big) showCheerZib(heard, 'cheer');
  afterCheer(heard, big ? 1050 : 420);
}

/* ── waiting for the praise to finish ────────────────────────
   The next round used to start on a timer, which meant a cheer
   that ran long — a recorded one, a name on the end, a slow
   phone — got cut off by its own game. Being talked over
   mid-sentence teaches a child that the talking does not matter.

   So two things have to happen before we move on: the cheer has
   to have been up long enough to read, and the sound has to have
   stopped. Whichever takes longer, wins.

   And a cap, because "wait for the sound" must never become
   "wait for ever": a file that stalls, a browser that eats the
   ended event, a tab put in the background — any of those would
   otherwise leave a five-year-old looking at a frozen screen.
   ──────────────────────────────────────────────────────────── */
const CHEER_CAP = 7000;
function afterCheer(heard, floor){
  const at = run.i, where = run.node;
  const jobs = [ new Promise(r => setTimeout(r, floor)) ];
  if(heard && heard.then){
    jobs.push(Promise.race([
      Promise.resolve(heard).catch(()=>{}),
      new Promise(r => setTimeout(r, CHEER_CAP))
    ]));
  }
  Promise.all(jobs).then(() => {
    /* the child left the place while it was playing */
    if(run.i !== at || run.node !== where) return;
...
/* ── Zib on the cheer screen ─────────────────────────────────
   Mounted the first time he is needed and kept after that, so
   there is no SVG being built between rounds. mascot.js is
   optional the same way Profiles is — without it the cheer is
   exactly what it was before, emoji and all.               */
let zCheer = null;
function showCheerZib(heard, mood){
  if(typeof Zib === 'undefined') return;
  const host = $('cheerZib'); if(!host) return;
  if(!zCheer) zCheer = Zib.mount(host, { size: 128, lead: false, mood: 'idle' });
  host.classList.add('on');
  Zib.celebrate(zCheer, heard, mood);
}
function hideCheerZib(){
  const host = $('cheerZib'); if(host) host.classList.remove('on');
  if(zCheer){ zCheer.mouth(false); zCheer.mood('idle') }
}
function miss(el){
  if(run.busy) return;
...
function finishNode(){
  ...
  if(first){
    /* Two lines, one after the other, and the second one waits —
       the pause used to be a guess at how long the first would
       take and a recorded cheer with a name on it overran it. */
    Promise.resolve(WLAudio.cheer('You did it', 0.8, 1.2))
      .catch(()=>{})
      .then(() => new Promise(r => setTimeout(r, 320)))
      .then(() => WLAudio.line('You did it! A new chapter for your storybook.', 0.8, 1.2));
  }else{
...
function stimulusCard(target, hint){
  ...
  return `<div class="say-card">
      <div class="big-pic" onclick="WLAudio.word('${w}')">${picHTML(target)}</div>
...
sound(r){
  ...
  stage(
    `<div class="say-card">
       ...
       <p class="say-hint">${sndLabel(r.answer)} … like <b>${r.key.w.toLowerCase()}</b> ${picHTML(r.key,'inline')}</p>
     </div>`,
...
beginSound(r){
  ...
  stage(
    `<div class="say-card">
       <div class="big-pic" onclick="WLAudio.word('${r.target.w}')">${picHTML(r.target)}</div>
  ...
  stage(
    ...
    `<div class="grid2">${r.opts.map(v=>
      `<div class="pic-card blank${v.w===r.answer?' is-correct':''}" role="button" tabindex="0"
        ...
        <div class="e">${picHTML(v)}</div><div class="w">${v.w.toLowerCase()}</div></div>`).join('')}</div>`);
...
listen(r){
  ...
  stage(
    ...
    `<div class="grid2">${r.opts.map(v=>
      `<div class="pic-card blank${v.w===r.answer?' is-correct':''}" role="button" tabindex="0"
        onclick="picTap(this,'${v.w}','${r.answer}')"><div class="e">${picHTML(v)}</div><div class="w">${v.w.toLowerCase()}</div></div>`).join('')}</div>`);
  setTimeout(()=>WLAudio.word(r.target.w), 320);
},
...
match(r){
  ...
  const pics = shuffle(r.pairs).map(p =>
    `<div class="match-item pic" role="button" tabindex="0" data-l="${p.l}" onclick="matchTapPic(this,'${p.l}','${p.pic.w}')">${picHTML(p.pic)}</div>`).join('');
...
blendIt(r){
  ...
  stage(
    ...
    `<div class="grid3" id="blendOpts" style="opacity:.3;pointer-events:none">${r.opts.map(v=>
      `<div class="pic-card${v.w===r.answer?' is-correct':''}" role="button" tabindex="0"
        onclick="picTap(this,'${v.w}','${r.answer}')">
        <div class="e">${picHTML(v)}</div></div>`).join('')}</div>`);
},
...
sentence(r){
  ...
  stage(
    `<div class="say-card">
       <div class="big-pic" onclick="sentenceSay()">${picHTML({w:r.picW, e:r.pic})}</div>
...
pickWord(r){
  ...
  stage(
    `<div class="say-card">
       <div class="big-pic" onclick="WLAudio.word('${r.answer}')">${picHTML({w:r.picW, e:r.pic})}</div>
...
readLine(r){
  ...
  stage(
    ...
    `<div class="grid3">${r.opts.map(v=>
      `<div class="pic-card blank${v.w===r.answer?' is-correct':''}" role="button" tabindex="0"
        onclick="picTap(this,'${v.w}','${r.answer}')"><div class="e">${picHTML(v)}</div></div>`).join('')}</div>`);
...
function skipRound(label){
  ...
  $('cheerE').textContent = '👍';
  $('cheerE').classList.add('small');
  $('cheerT').textContent = 'Good try';
  $('cheerW').textContent = label ? String(label).toLowerCase() : '';
  $('cheer').classList.add('on');

  /* A skipped one still gets Zib, but proud rather than cheering
     — the child asked for a way out and took it, which is not
     nothing, and it is not a win either. */
  const heard = WLAudio.line('Have another go', 0.85, 1.15);
  showCheerZib(heard, 'proud');
  afterCheer(heard, 700);
}
