...
.zib.tappable{pointer-events:auto;cursor:pointer}
.zib.tappable:focus-visible{outline:3px solid #778BEB;outline-offset:6px;border-radius:20px}
.zib.tappable .zib-svg{transition:transform .14s}
.zib.tappable:active .zib-svg{transform:scale(.94)}
.zib.bumped .zib-svg{animation:zibBump .45s ease}
@keyframes zibBump{0%,100%{transform:translateY(0)}35%{transform:translateY(-9px) scale(1.04)}}
...
@media (prefers-reduced-motion:reduce){
  ...
  function make(host, opts) {
    ...
    const inst = {
      ...
      /* Somebody else is playing the sound — Word Land plays the
         praise itself so it can wait for it to finish — and Zib
         still has to look like the one saying it. This is the
         mouth on its own, with no line and no bubble. */
      mouth(on) { root.classList.toggle('talking', on !== false); return inst },

      hideBubble() { bubble.classList.remove('on'); return inst },
      quiet() { bubble.classList.remove('on'); settle(inst); return inst },
    ...
    /* ── tap him to hear it again ──────────────────────────
       Cheaper than a separate help button, and a child works
       it out on their own: the thing that said it is the thing
       you press to hear it. Zib becomes the button, so nothing
       goes inside the bubble.                                */
    if (opts.tap) {
      inst.noReplay = true;
      root.classList.add('tappable');
      root.setAttribute('role', 'button');
      root.tabIndex = 0;
      root.setAttribute('aria-label', opts.tapLabel || 'Say that again');
      const go = () => {
        /* Mid-sentence, a tap only gets a bounce. Cutting him off to
           repeat the thing he is already saying is just confusing, and
           a child who gets no response at all thinks he is broken. */
        if (inst.busy) {
        ...
        const l = inst.lastSaid;
        if (l) say(l.text, { mood: l.mood || 'happy', on: inst, then: 'idle', keep: l.keep });
      };
  ...
  /* Whatever ends a line — it finished, it was interrupted, the child
     left the screen — it has to settle exactly once. A sequence waits on
     these promises, so one that never resolves stalls the flow for good. */
  function settle(inst) {
    clearTimeout(inst.timer);
    inst.busy = false;
    inst.root.classList.remove('talking');
    const done = inst.pending;
    inst.pending = null;
    if (done) done(inst);
  }
  ...
  function say(text, opts) {
    ...
    settle(inst);                    // whatever he was mid-way through, let it go
    last = { text, mood: opts.mood };
    inst.lastSaid = { text, mood: opts.mood, keep: opts.keep };
    inst.busy = true;

    if (opts.mood) inst.mood(opts.mood);
    inst.root.classList.add('talking');
    inst.bubbleText(text, inst.noReplay ? false : opts.replay);

    const hold = opts.hold || dwell(text);
    const heard = speak(text);

    return new Promise(resolve => {
      inst.pending = resolve;
      inst.timer = setTimeout(() => {
        if (opts.keep !== true) inst.hideBubble();
  ...
  const LINES = [
    ...
    /* ── the hub ── */
    { t: "Who's playing today?",          g: 'Choosing a game', tier: 1 },
    { t: "Tap your picture.",             g: 'Choosing a game', tier: 2 },
    { t: "Hello",                         g: 'Choosing a game', tier: 1, name: true, have: true },
  ...
  /* ── cheering along with a recording ──────────────────────
     The game plays the praise itself, because it needs to know
     when the sound stops. Zib's job is to be visibly the one
     saying it: he pops up, he beams, his mouth moves for exactly
     as long as the recording lasts.

         Zib.celebrate(inst, WLAudio.praise('Well done'))

     `heard` may be a promise, or nothing at all — a quick tick
     with no sound still gets a smile, it just gets a short one. */
  function celebrate(inst, heard, mood) {
    if (!inst) return heard || Promise.resolve();
    inst.show();
    inst.flash(mood || 'cheer', 2600);
    inst.mouth(true);
    const shut = () => { try { inst.mouth(false) } catch (e) {} };
    if (heard && heard.then) heard.then(shut, shut);
    else setTimeout(shut, 900);
    return heard || Promise.resolve();
  }

  return {
    mount: make, say, sequence, helper, script, file, celebrate,
