...
/* mark what already exists so the generator never overwrites a
   recording you made with your own voice */
...
console.log('  already recorded ' + spec.filter(s => s.exists).length);
console.log('  held back        ' + skipped.length + '  (' + [...new Set(skipped.map(s => s.path.split('/')[0]))].join(', ') + ')');
console.log('  to synthesise    ' + todo.length);
console.log('  characters       ' + todo.reduce((a, s) => a + s.text.length, 0));
