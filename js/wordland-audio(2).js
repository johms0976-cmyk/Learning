...
const WLAudio = (function () {
  ...
  function candidates(kind, name) {
    return packs().map(v => pathIn(v, kind, name));
  }
  ...
  /* ── is this child's name recorded, and where? ────────────
     ...
     ────────────────────────────────────────────────────────── */
  async function namePart(seat, who) {
    const i = seat || playerSlot();
    if (i && await firstAvailableSoon(candidates('players', 'player' + i))) {
      return ['players', 'player' + i];
    }
    const n = nameSlug(who == null ? playerName() : who);
    if (n && await firstAvailableSoon(candidates('names', n))) {
      return ['names', n];
    }
    return null;
  }
  ...
  /* For the hub, while a grown-up is typing: can we say this one
     ...
     they already have. */
  async function hasName(who) {
    const n = nameSlug(who);
    if (!n) return false;
      ...
      return new Promise(res => {
        ...
        try {
          if (typeof WLVoices !== 'undefined') {
            u.lang = WLVoices.lang();
            const sv = WLVoices.speechVoice();
            if (sv) u.voice = sv;
          }
        } catch (e) {}
  ...
  async function say(kind, name, fallbackText, rate, pitch) {
    ...
    const u = await firstAvailableSoon(candidates(kind, name));
    if (u) {
  ...
  async function seqFiles(parts, mine) {
  ...
  async function seq(parts, fallbackText, rate, pitch) {
    ...
    stop();
    const mine = ++token;
    if (await seqFiles(parts, mine)) return;
    if (mine !== token) return;
  ...
  return {
    ...
    player: async function (n, who) {
      const part = await namePart(n, who);
      if (!part) return;
      return say(part[0], part[1], '', 0.8, 1.15);
    },
    ...
    cheer: async function (text, rate, pitch) {
      if (muted) return;
      const part = await namePart();
      if (part) {
        stop();
        const mine = ++token;
        if (await seqFiles([['praise', slug(text)], part], mine)) return;
        if (mine !== token) return;
      }
      /* No name, or no rising recording to run into it — so say the
         line finished and on its own. phrases/ first, computer
         voice after that. */
      return say('phrases', slug(text), text, rate || 0.8, pitch || 1.15);
    },
